...
def gate_decision(
    ...
    if quarantine == "block":
        return out("block", "1.quarantine_block", "Quarantine blocked.")
    # 2 — a required harness did not complete -> fail closed (A8)
    if not required_ran:
        return out("block", "2.required_harness_not_run",
                   "A required harness did not complete (A8 fail-closed).")
    ...
    # 8 — any high finding -> warn
    if any(f.severity == "high" for f in findings):
        return out("warn", "8.high_finding", "High-severity finding.")
    # 9 — default
    return out("approve", "9.default", "No blocking conditions.")
